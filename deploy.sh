...
kubectl apply -f k8s 
kubectl set image deployments/server-deployment server=ishwo/multi-server:$SHA 
kubectl set image deployments/client-deployment client=ishwo/multi-client:$SHA
kubectl set image deployments/worker-deployment worker=ishwo/multi-server:$SHA
